...
**核心特性**: 消息队列架构 | 独立执行器 | 可视化脚本编辑 | 多框架支持 | RabbitMQ用户集成
...
| **测试人员** (tester) | 2 | 查看、创建脚本、执行测试 + **账号管理(仅自己)** |
| **访客** (guest) | 1 | 仅查看权限 |
...
**方式一：用户级集成（推荐）**
1. 管理员在「账号角色管理」中为用户启用 RabbitMQ 功能
2. 系统自动为该用户创建 RabbitMQ 账号（随机密码）
3. 用户登录平台后在「账号角色管理」中点击「RabbitMQ配置」按钮查看配置
4. 执行机使用用户的 RabbitMQ 账号连接（支持远程）

**方式二：独立账号管理**
1. 超级管理员在「账号角色管理」页面的「RabbitMQ用户管理」区域创建账号
2. 平台通过RabbitMQ HTTP API创建用户并设置权限
3. 管理员将用户名/密码提供给执行机用户
...
| `/api/users/{id}/get_rabbitmq_config/` | GET | 获取用户RabbitMQ配置 | 本人+管理员 |
| `/api/users/{id}/toggle_rabbitmq/` | POST | 启用/禁用用户RabbitMQ | 超级管理员 |
| `/api/users/role_list/` | GET | 获取角色列表 | 测试人员+ |
...
| `/api/users/{id}/get_rabbitmq_config/` | GET | 获取用户RabbitMQ配置 | 本人+管理员 |
| `/api/users/{id}/toggle_rabbitmq/` | POST | 启用/禁用用户RabbitMQ | 超级管理员 |
| `/api/users/create_rabbitmq_user/` | POST | 创建独立RabbitMQ用户 | 超级管理员 |
...
远程执行机需要使用专用的 RabbitMQ 用户账号连接：

**方式一：使用集成的 RabbitMQ 配置（推荐）**
1. 使用用户账号登录平台
2. 进入「账号管理」页面
3. 点击对应账号行的「RabbitMQ配置」按钮
4. 复制显示的配置信息（用户名、密码、主机地址、端口）
5. 在执行机客户端配置向导中填写这些信息

**方式二：创建独立 RabbitMQ 用户**
1. 使用超级管理员账号登录平台
...
为简化执行机配置流程，平台已将平台用户账号与 RabbitMQ 用户账号进行集成。

**用户级别 RabbitMQ 配置**：
- 每个平台用户可独立启用 RabbitMQ 功能
- 启用后自动创建对应的 RabbitMQ 用户账号
- 用户可在「账号管理」页面点击「RabbitMQ配置」按钮查看配置信息
- 配置信息包括：用户名、密码、主机地址、端口
- 普通用户可查看自己的配置，管理员可查看所有人的配置

**管理员操作**（仅超级管理员）：
- **创建用户时启用 RabbitMQ**：在新建用户对话框中勾选「RabbitMQ」选项
- **开关 RabbitMQ 功能**：在用户列表点击「启用/禁用」按钮
- **删除用户时**：自动删除对应的 RabbitMQ 用户账号

**高级 RabbitMQ 管理**（仅超级管理员）：
- 进入「账号角色管理」→「角色管理」标签页
- 在「RabbitMQ 用户管理」区域可进行批量管理操作
- 支持创建独立 RabbitMQ 用户、修改密码、删除用户

**配置说明**：
- 本地配置（执行机与服务器同电脑）：主机地址使用 `127.0.0.1`
- 远程配置（执行机与服务器不同电脑）：主机地址使用服务器实际 IP
- 系统会自动获取服务器的网络地址作为默认值
